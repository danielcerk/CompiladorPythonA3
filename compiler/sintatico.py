...
def adicionar_nos(node, parent_id=None):
    ...
    if isinstance(node, While):
        # nó marcador para condição
        cond_id = novo_id()
        dot.node(cond_id, "condição", shape="ellipse", style="filled", fillcolor="lightgrey")
        dot.edge(node_id, cond_id)
        adicionar_nos(node.condicao, cond_id)

        # nó marcador para bloco de comandos
        bloco_id = novo_id()
        dot.node(bloco_id, "bloco", shape="ellipse", style="filled", fillcolor="lightgrey")
        dot.edge(node_id, bloco_id)
        for stmt in node.corpo:
            adicionar_nos(stmt, bloco_id)
...
print("Gerado: ast_mini_python.png")
dot.render("ast_mini_python", format="png", cleanup=True)
